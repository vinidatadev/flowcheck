...
const selectedUsers = computed(() => {
  return props.users.filter(user => 
    user.nome_usuario && props.modelValue.includes(user.nome_usuario)
  )
})
...
const isSelected = (user: User): boolean => {
  return user.nome_usuario ? props.modelValue.includes(user.nome_usuario) : false
}

const selectUser = (user: User) => {
  if (!user.nome_usuario) return
  
  if (isSelected(user)) {
    // Remove user
    const newValue = props.modelValue.filter(name => name !== user.nome_usuario)
    emit('update:modelValue', newValue)
  } else {
    // Add user
    emit('update:modelValue', [...props.modelValue, user.nome_usuario])
  }
  
  // Limpar busca mas manter dropdown aberto
  searchQuery.value = ''
}
